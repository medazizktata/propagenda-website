...
import type { CaseStudyListRow } from '@/types/cms';
import { deleteCaseStudies } from '@/app/admin/(protected)/case-studies/actions';
...
export function CaseStudiesTable({ rows }: { rows: CaseStudyListRow[] }) {
  ...
  const { columns, bulkActions } = useMemo(() => {
    const copySlugs = async (selected: CaseStudyListRow[]) => {
      await navigator.clipboard.writeText(selected.map((row) => row.slug).join('\n'));
    };

    const deleteItems = async (selected: CaseStudyListRow[]) => {
      const result = await deleteCaseStudies(selected.map((row) => row.id));
      if (!result.ok) {
    ...
    const dataColumns: TableColumn<CaseStudyListRow>[] = [
      {
        key: 'title',
        label: 'Title',
        sortable: true,
    ...
    const exportColumns: TableColumn<CaseStudyListRow>[] = [
      { key: 'select', label: '', pinned: 'start', render: () => null },
      ...dataColumns,
    ];

    const exportCsv = (selected: CaseStudyListRow[]) => {
      exportRowsToCsv({
        rows: selected,
        columns: exportColumns,
        filename: selected.length === 1 ? `${selected[0]!.slug}.csv` : 'case-studies-export.csv',
      });
    };

    const rowActions: TableAction<CaseStudyListRow>[] = [
      {
        id: 'edit',
        label: 'Edit',
        icon: <Pencil className="size-4" />,
        href: (row) => `/admin/case-studies/${row.slug}`,
      },
      {
        id: 'copy-slug',
        label: 'Copy slug',
    ...
    const tableColumns: TableColumn<CaseStudyListRow>[] = [
      { key: 'select', label: '', pinned: 'start', render: () => null },
      ...dataColumns,
      {
        key: 'actions',
    ...
    const bulk: BulkAction<CaseStudyListRow>[] = [
  ...
  return (
    <DataTable
      storageKey="admin.case-studies"
      rows={rows}
      columns={columns}
      bulkActions={bulkActions}
      quickFilters={quickFilters}
      emptyMessage="No case studies match your filters."
    />
  );
}
