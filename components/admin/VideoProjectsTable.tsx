...
import type { VideoProjectListRow } from '@/types/cms';
import { deleteVideoProjects } from '@/app/admin/(protected)/video/actions';
...
export function VideoProjectsTable({ rows }: { rows: VideoProjectListRow[] }) {
  ...
  const quickFilters = useMemo(
    () => [
      { id: 'all', label: `All (${rows.length})`, scopeFields: ['status'], rules: [] },
      {
        id: 'published',
  ...
  const { columns, bulkActions } = useMemo(() => {
    const copySlugs = async (selected: VideoProjectListRow[]) => {
      await navigator.clipboard.writeText(selected.map((row) => row.slug).join('\n'));
    };

    const deleteItems = async (selected: VideoProjectListRow[]) => {
      const result = await deleteVideoProjects(selected.map((row) => row.id));
      if (!result.ok) {
    ...
    const dataColumns: TableColumn<VideoProjectListRow>[] = [
      {
        key: 'title',
        label: 'Title',
        sortable: true,
    ...
    const exportColumns: TableColumn<VideoProjectListRow>[] = [
      { key: 'select', label: '', pinned: 'start', render: () => null },
      ...dataColumns,
    ];

    const exportCsv = (selected: VideoProjectListRow[]) => {
      exportRowsToCsv({
        rows: selected,
        columns: exportColumns,
        filename: selected.length === 1 ? `${selected[0]!.slug}.csv` : 'video-work-export.csv',
      });
    };

    const rowActions: TableAction<VideoProjectListRow>[] = [
      {
        id: 'edit',
        label: 'Edit',
        icon: <Pencil className="size-4" />,
        href: (row) => `/admin/video/${row.slug}`,
      },
      {
        id: 'copy-slug',
        label: 'Copy slug',
    ...
    const tableColumns: TableColumn<VideoProjectListRow>[] = [
      { key: 'select', label: '', pinned: 'start', render: () => null },
      ...dataColumns,
      {
        key: 'actions',
    ...
    const bulk: BulkAction<VideoProjectListRow>[] = [
      { id: 'copy-slugs', label: 'Copy slugs', run: copySlugs },
      { id: 'export', label: 'Export CSV', run: exportCsv },
      { id: 'delete', label: 'Delete', destructive: true, confirm: 'Confirm delete', run: deleteItems },
    ];
  ...
  return (
    <DataTable
      storageKey="admin.video"
      rows={rows}
      columns={columns}
      bulkActions={bulkActions}
      quickFilters={quickFilters}
      emptyMessage="No video projects match your filters."
    />
  );
}
